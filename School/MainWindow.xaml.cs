        ...
        private void studentsList_KeyDown(object sender, KeyEventArgs e)
        {
            switch (e.Key)
            {
                ...
                case Key.Insert:
                    ...
                    MessageBox.Show(e.Key.ToString() + " was pressed", "Button pressed: "); //Shows a messagebox that displays the keydown event that was triggered.
                    StudentForm sfn = new StudentForm(); //Initializes a new StudentForm called sfn
                    sfn.Title = "New student for class" + teacher.Class; // Sets the title of the studentForm
                    if (sfn.ShowDialog().Value==true) // Works the same if '==true'is left out.
                    {
                        Student newStudent = new Student(); // Creates a new student object called newStudent
                            //Sets the properties of the student object per data typed in by user in the student form.
                        newStudent.FirstName = sfn.firstName.Text;
                        newStudent.LastName = sfn.lastName.Text;
                        newStudent.DateOfBirth = DateTime.Parse(sfn.dateOfBirth.Text); //parses the string value of the textbox into a DateTime.

                        this.teacher.Students.Add(newStudent); //Selects the current teacher (using this. keyword) and uses the students.add method to add the newStudent (overloaded with the newStudent student object)
                        saveChanges.IsEnabled = true;
                    }
